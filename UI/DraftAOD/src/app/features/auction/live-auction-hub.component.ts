...
export class LiveAuctionHubComponent implements OnInit, OnDestroy {
  ...
  readonly secondsRemaining=computed(()=>{const ends=this.currentLot()?.endsAtUtc;if(!ends)return 0;return Math.max(0,Math.ceil((new Date(ends).getTime()-this.now())/1000));});
  ...
  resume():void {const lot=this.currentLot();if(lot)this.live.resume(lot.id,this.duration()).subscribe({next:()=>this.load(false),error:e=>this.error.set(this.errorText(e,'The lot could not be resumed.'))});}
