...
export class LiveAuctionHubComponent implements OnInit, OnDestroy {
  ...
  readonly secondsRemaining=computed(()=>{const lot=this.currentLot();if(!lot||lot.state==='Closed'||lot.state==='Cancelled')return 0;if(lot.state==='Paused')return lot.pausedRemainingSeconds??0;const ends=lot.endsAtUtc;if(!ends)return 0;return Math.max(0,Math.ceil((new Date(ends).getTime()-(this.now()+this.clockOffsetMs()))/1000));});
