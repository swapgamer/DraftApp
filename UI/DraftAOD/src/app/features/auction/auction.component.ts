...
export class AuctionComponent {
  ...
  assign(team:AuctionTeam,player:AuctionPlayer,price:number):void {if(price<=0){this.error.set('Enter a positive sold price before assigning a player.');return;}this.auction.assign(team.id,player.id,price).subscribe({next:()=>this.load(),error:error=>this.error.set(this.errorText(error,'The player could not be assigned.'))});}
  ...
  private errorText(error:any,fallback:string):string {return typeof error?.error==='string'?error.error:fallback;}
}
