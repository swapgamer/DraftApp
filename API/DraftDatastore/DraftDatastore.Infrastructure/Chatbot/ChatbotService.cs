    ...
    ILogger<ChatbotService> logger) : IChatbotService
{
    ...
    private static async Task<(string Intent, List<Player> Players, string[]? PositionCodes)?> TryGetDirectResultAsync(IQueryable<Player> query, string lower, CancellationToken ct)
    {
        ...
        if (lower.Contains("top") && positionCodes is not null)
            return ("TopByPosition", await query.Where(x => x.PlayerPositions.Any(p => positionCodes.Contains(p.Position.Code)))
                .OrderBy(x => x.PlayerPositions.Where(p => positionCodes.Contains(p.Position.Code)).Select(p => p.OverallRank ?? x.OverallRank).Min()).Take(5).ToListAsync(ct), positionCodes);
        if (lower.Contains("rank") && positionCodes is not null)
            return ("RankByPosition", await query.Where(x => x.PlayerPositions.Any(p => positionCodes.Contains(p.Position.Code)))
                .OrderBy(x => x.PlayerPositions.Where(p => positionCodes.Contains(p.Position.Code)).Select(p => p.OverallRank ?? x.OverallRank).Min()).Take(1).ToListAsync(ct), positionCodes);
